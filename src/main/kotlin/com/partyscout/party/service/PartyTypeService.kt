...
import com.partyscout.party.model.PartyTypeTaxonomy
...
class PartyTypeService {
    ...
    fun getKeywordsForPartyType(type: String): List<String> {
        return partyTypeTaxonomy.find { it.type == type }?.searchKeywords ?: emptyList()
    }

    fun getGooglePlacesTypesForPartyType(type: String): List<String> {
        return partyTypeTaxonomy.find { it.type == type }?.googlePlacesTypes ?: emptyList()
    }
    ...
    fun getTypicalDuration(type: String): String {
        return partyTypeTaxonomy.find { it.type == type }?.typicalDuration ?: "2 hours"
    }

    fun getSetting(type: String): String {
        return partyTypeTaxonomy.find { it.type == type }?.setting ?: "indoor"
    }
